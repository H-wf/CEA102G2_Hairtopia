		...
		<div class="row">
			...
			<div class="col-2">
				<div class="list-group">
					<!-- 這邊要填超連結跟class		 -->
					<a
						href="<%=request.getContextPath()%>/service/service.do?action=queryByDesNo&desNo=${desSession.desNo}"
						class="list-group-item list-group-item-action"> 服務項目管理 </a> <a
						href="<%=request.getContextPath()%>/reservation/res.do?action=queryByDesNo&desNo=${desSession.desNo}"
						class="list-group-item list-group-item-action"> 預約狀態管理 </a> <a
			...
			<div class="col-md-6">
				...
				<FORM METHOD="post"
					...
					<div class="card card-body">
						...
						<div class="form-group">
							<label for="desName">設計師名字</label> <input type="TEXT"
								name="desName" id="desName" class="form-control"
								value="${empty designerVO ? ' ' :designerVO.desName}" />
						</div>
						...
						<div class="container-fluid ScheduleTime">
										...
										<select class="custom-select custom-control-inline time">
											<c:forEach var="whatTime" begin="0" end="47">
												<fmt:formatNumber type="number"
													value="${((whatTime*30 -(whatTime*30%60)))/60}" var="hour" />
												<c:set var="select" value="${s.index *4}" />

												<c:set var="schedule"
													value="${x.substring(select,select+2)}" />
												<option value='${whatTime}'
													${whatTime == schedule ? "selected" : ""}>
										...
										<select class="custom-select custom-control-inline time">
											<c:forEach var="whatTime" begin="0" end="47">
												...
												<c:if test="${whatTime%6 ==0 && whatTime/6 != 7}">
													<c:set var="schedule"
														value="${x.substring(select+2,select+4)}" />
												</c:if>
							...
							<div class="form-group ScheduleTime">
								...
								<textarea id='desInfor' row="10" cols="48" name="desInfor"
									class="form-control" rows="10" size="45">${empty designerVO ? ' ' :designerVO.desInfor}</textarea>
							...
							<input type="hidden" name="salNo" size="45"
								value="${designerVO.salNo}"> <input type="hidden"
								name="desNo" value="${designerVO.desNo}"> <input
								type="hidden" name="desSchedule" id="desSchedule" value="1">
