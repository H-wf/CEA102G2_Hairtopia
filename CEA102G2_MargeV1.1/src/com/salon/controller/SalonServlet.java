	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("getOne_For_Display".equals(action)) { // 來自select_salon_page.jsp的請求
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req.getRequestDispatcher("/front-end/salon/select_salon_page.jsp");
		...
		if ("insert".equals(action)) { // 來自addSalon.jsp的請求
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					...
					RequestDispatcher failureView = req.getRequestDispatcher("/front-end/salon/addSalon.jsp");
					failureView.forward(req, res);
					return;
				}
		...
		if ("getOne_For_Update".equals(action)) { // 來自listAllSalon.jsp的請求
			...
			try {
				...
				RequestDispatcher failureView = req.getRequestDispatcher("/front-end/salon/listAllSalon.jsp");
				failureView.forward(req, res);
			}
		}
		...
		if ("update".equals(action)) { // 來自update_salon_input.jsp的請求
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					...
					RequestDispatcher failureView = req.getRequestDispatcher("front-end/salon/update_salon_input.jsp");
					failureView.forward(req, res);
					return; // 程式中斷
				}
...
//			
			try {
				...
				if (!errorMsgs.isEmpty()) {
					...
					RequestDispatcher failureView = req.getRequestDispatcher("/front-end/designer/addDesAndSal.jsp");
					failureView.forward(req, res);
					return;
				}
				...
				req.setAttribute("newDesReq", "已收到申請，請靜待審核。");
